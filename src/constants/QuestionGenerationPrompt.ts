// Types
export type QuestionType = 'Flash Cards' | 'Multiple Choice' | 'Multiple True-False';
...
const getBasePrompt = (questionCount: number, focusAreas?: string[], questionType?: QuestionType): string => `
Analyze the document thoroughly. Generate ${questionCount} unique and new ${
  questionType === 'Flash Cards' 
    ...
    ? 'multiple true-false' 
    : 'multiple-choice'
} questions based on key concepts. ${DIFFICULTY_PROMPT}
...
const getSpecificPrompt = (questionType: QuestionType, includeCaseStudies: boolean): string => {
  if (questionType === 'Flash Cards') return getFlashCardPrompt();
  if (questionType === 'Multiple True-False') return getMultipleTrueFalsePrompt();
  return includeCaseStudies ? getCaseStudyPrompt() : getDirectQuestionPrompt(questionType);
};
